...
```{r getting data, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
```{r summarise the data, anchor="Table", echo=T}
...
You'll see above that I used a labelled for the table, I did that by adding `anchor="table"` to the start of the chunk (along with the name `summarise the data`). Now, I can use `figr("summarise the data", "Table")` to refer to it like this `r figr("summarise the data")`. I haven't worked out if you can get it to output the whole caption (e.g. Table 1: Caption name here).  
...
The ggplot package makes much nicer figures, like that shown in `r figr("pretty histograms", type = "Figure")`. 
...
Your referenec list should go here